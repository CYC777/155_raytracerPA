class Scene {
	...
	getColorForRay(ray,depth){
		...
		if (intersection.isEmpty()){
			...
			let theColor = this.calculateColor(p,n,ray,mat,textureColor)
	...
	calculateColor(point, normal, ray, mat, textureColor){
		if (mat.nolighting) return textureColor
		let theColor= this.globalAmbient
		const eye = ray.p.subtract(point).normalize()
		const distance = ray.p.subtract(point).length()

		for(let light of this.lights){
			if (this.reaches(light, point)){
				const ambient = light.ambient(point).times(mat.ambient)
		...
		if (distance > this.fogDistance) {
			return Color.WHITE;
		} else {
			fog_color.r = (1 - theColor.r) * distance / this.fogDistance;
			fog_color.g = (1 - theColor.g) * distance / this.fogDistance;
			fog_color.b = (1 - theColor.b) * distance / this.fogDistance;
		}

		theColor.add(fog_color);
